...
public class ReSolution {
    ...
    public static void main(String[] args) {
        System.out.println(new ReSolution().characterReplacement("ABAB", 0) == 1);
        System.out.println(new ReSolution().characterReplacement("ABAB", 1) == 3);
        System.out.println(new ReSolution().characterReplacement("ABAB", 2) == 4);
        System.out.println(new ReSolution().characterReplacement("AABABBA", 1) == 4);
        System.out.println(new ReSolution().characterReplacement("AAAA", 2) == 4);
        System.out.println(new ReSolution().characterReplacement("ABCDE", 1) == 2);
        System.out.println(new ReSolution().characterReplacement("ABBB", 2) == 4);
        System.out.println(new ReSolution().characterReplacement("BAAAB", 2) == 5);
        System.out.println(new ReSolution().characterReplacement("AABCCCBAABB", 4) == 7);
    }
    ...
    int getLongestReplaceableRepeatingFor(List<Integer> jumpList, int n, int k) {
        ...
        for (; windowEnd < m; windowEnd++) {
            ...
            // If we can't expand without exceeding the bandwidth then try to maximize how far the current window can reach with the remaining bandwidth before advancing the window.
            if (bandwidth < consumptionRequirement) {
                int extraJuice = juiceAvailableFromRemainingBandwidth(jumpList, bandwidth, windowStart, windowEnd, n);
            ...
            while (bandwidth < 0) {
                int freeableBandwidth = jumpList.get(windowStart + 1) - jumpList.get(windowStart) - 1;
    ...
    int juiceAvailableFromRemainingBandwidth(List<Integer> jumpList, int bandwidth, int windowStart, int windowEnd, int n) {
